...
router.post('/start', async (req, res) => {
  try {
    ...
    const locale = supportedLocales.includes(rawLocale) ? rawLocale : 'it';
    let cleanedContent = sanitizeUserText(content);
    if (manualIndexSummary && String(manualIndexSummary).trim()) {
    ...
    const conversationHistory = [
      {
        role: 'user',
        content: `Start an oral exam based ONLY on the uploaded material. Detected topic: ${derivedTopic}. Mode: ${examMode}. Target questions: ${targetQuestions}. Language: ${locale}. First question must be specific, grounded in the material, and non-generic.`,
      },
    ];
