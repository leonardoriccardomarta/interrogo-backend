...
export function resolveLanguage(locale = 'it') {
  return LOCALE_LANGUAGE[locale] || LOCALE_LANGUAGE.it;
}
...
export class InterrogoAIService {
  ...
  buildSystemPrompt(personality, content, locale = 'it') {
...
- QUESTION QUALITY: ask about specific concepts from the material; avoid yes/no; no generic questions; never repeat a question already asked`;
  ...
  async generateQuestion(content, conversationHistory, difficulty, personality, locale = 'it') {
    if (!this.groqApiKey) {
      throw new Error('Groq API key not configured');
    }

    try {
      const systemPrompt = this.buildSystemPrompt(personality, content, locale);
...
- evaluate ONLY within the provided material scope: no external knowledge.`;
  ...
  async generateFlashcards(content, locale = 'it', cardCount = 15) {
    if (!this.groqApiKey) {
      throw new Error('Groq API key not configured');
    }

    const language = resolveLanguage(locale);
    const prompt = `Create exactly ${cardCount} study flashcards from the material below.
Respond ONLY with valid JSON array (no markdown):
[
  {"front": "question or term", "back": "concise answer", "chapter": "optional section", "difficulty": 5}
]

Rules:
- Language: ${language}
- Front: short question, term, or prompt (max 120 chars)
- Back: clear answer grounded in material (max 280 chars)
- difficulty: integer 1-10
- Cover definitions, key facts, formulas, dates, cause-effect
- No duplicate fronts
- Stay within material scope
    ...
    try {
      const response = await axios.post(
        `${this.groqBaseUrl}/chat/completions`,
        {
          model: 'llama-3.1-8b-instant',
          messages: [
            { role: 'system', content: 'You create educational flashcards. Respond ONLY with a JSON array.' },
      ...
      const raw = response.data.choices[0]?.message?.content;
      if (!raw) throw new Error('Empty flashcard response');
      ...
      return parsed.slice(0, cardCount).map((card, idx) => ({
        front: String(card.front || `Card ${idx + 1}`).slice(0, 300),
        back: String(card.back || '').slice(0, 500),
        chapter: card.chapter ? String(card.chapter).slice(0, 120) : null,
        difficulty: Math.min(10, Math.max(1, Number(card.difficulty) || 5)),
      }));
    } catch (error) {
